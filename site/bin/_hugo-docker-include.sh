...
# absolute path to the ActiveMQ workspace, for the volume mount
WORKSPACE="$(pwd)"
export WORKSPACE

# Mount options for the ActiveMQ workspace / slight difference between Podman + docker / Linux + macOS.
...
case "$(uname)" in
  Linux)
    ...
    if [[ -z ${CI} ]] ; then
      ...
      # Needed for podman on Linux, so it doesn't "chown" the ActiveMQ workspace to an ephemeral UID (Linux only)
...
function cleanup() {
  ...
  $COMPOSE \
    ...
    down > /dev/null 2>&1 || true
  $DOCKER container rm -f activemq_site-hugo-1 activemq_site-hugo_publish_localhost8080-1 activemq_site-hugo_publish_apache-1 > /dev/null 2>&1 || true
  $DOCKER volume rm -f activemq_site_resources activemq_site_workspace activemq_site_hugo_cache > /dev/null 2>&1 || true
}
