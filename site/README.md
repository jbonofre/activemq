...
Just run the following command to let Hugo serve files from the ActiveMQ `site/`.
```bash
...
This builds the static Apache ActiveMQ site into `site/build/localhost-site` to be served using a http daemon.
...
## Building the Apache ActiveMQ site

This builds the static Apache ActiveMQ site into `site/build/apache-site`. The base URL will be
`https://activeemq.apache.org/` (as configured in `hugo.yaml`).
(Note: static sites generated by Hugo do **not** work when opening the files from the local file system!)
...
The `/releases` folder would _not_ be in the main ActiveMQ repository (at least not in any code branch). It would be
mounted either via `git worktree` or `git clone`. Building and testing the site locally works without the `/releases`
folder. For developers, it is probably much easier to have a separate branch in the main ActiveMQ GitHub repository
